...
def evaluate_evidence_quality(startup_name: str, search_results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if len(search_results) >= 10:
        ...
        reasons.append("fewer than 10 search results")
    ...
    if trusted_hits >= 2:
        ...
        reasons.append("no trusted-source coverage")

    if startup_relevance_hits >= 4:
        score += 2
    else:
        reasons.append("weak startup-specific relevance")
...
def ensure_config_ok() -> None:
    api_key = os.environ.get("OPENAI_API_KEY") or os.environ.get("DEEPINFRA_API_KEY")
    if not api_key:
        st.error("Missing API key. Please set OPENAI_API_KEY or DEEPINFRA_API_KEY in your terminal.")
        st.stop()
...
def create_openai_client() -> OpenAI:
    ...
    if not api_key:
        raise RuntimeError("Missing OPENAI_API_KEY/DEEPINFRA_API_KEY.")
    base_url = os.environ.get("OPENAI_BASE_URL", "https://api.deepinfra.com/v1/openai")
    return OpenAI(api_key=api_key, base_url=base_url)


def call_llm(prompt: str) -> str:
    client = create_openai_client()
    model_name = os.environ.get("LLM_MODEL_NAME", "meta-llama/Meta-Llama-3-70B-Instruct")
    response = client.chat.completions.create(
        model=model_name,
        messages=[
            {
