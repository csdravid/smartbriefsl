...
def lock_low_confidence_sections(text: str, evidence: Dict[str, Any]) -> str:
    # Simplified mode: never lock sections.
    return text
...
def evaluate_evidence_quality(
    ...
    """Lightweight diagnostics only (no hard quality gate)."""
    trusted_domains = [
        "startupticker.ch",
        "venturelab.swiss",
        "startup.ch",
    ...
    # Keep thresholds for display only; do not use them to block generation.
    is_balanced = reliability_mode.lower() == "balanced"
    min_trusted_hits = 1 if is_balanced else 1
    min_startup_relevance_hits = 1 if is_balanced else 1
    min_results = 1
